...
export function CoursesCatalog() {
  ...
  const selectedPath = catalog.paths.find((item) => String(item.id) === pathId);
  const list = catalog.courses.filter((item) => !pathId || String(item.pathId) === pathId);
  ...
  return (
    <div className="page-width content-page">
      ...
      <nav className="filter-bar" aria-label="课程路径筛选"><Link className={!pathId ? "active" : ""} href="/courses">全部</Link>{catalog.paths.map((path) => <Link className={String(path.id) === pathId ? "active" : ""} href={`/courses?pathId=${path.id}`} key={path.id}>{path.name}</Link>)}</nav>
